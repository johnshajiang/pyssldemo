...
# Generate X.509 version 3 extension file for end entity.
cat > ee.ext << EOF
...
# Generate an empty CRL, namely no certificate is revoked.
openssl ca -gencrl -config conf -name crl -cert CA_RSA.cer.tmp -keyfile CA_RSA.key.tmp -crldays 3650 -md sha256 -out CA_RSA.crl.tmp
# Merge CA RSA certificate and the associated CRL into a single file.
openssl x509 -text -in CA_RSA.cer.tmp CA_RSA.crl.tmp > CA_RSA.cer
...
# Generate server and client end entity certificates with CRL.
touch index
